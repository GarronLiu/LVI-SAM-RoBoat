...
void pubLatestOdometry(const Eigen::Vector3d &P, const Eigen::Quaterniond &Q, const Eigen::Vector3d &V, const std_msgs::Header &header, const int& failureId)
{
    ...
    tf::Quaternion q_lidar_to_cam = tf::createQuaternionFromRPY(L_I_RX,L_I_RY,L_I_RZ);//动态欧拉角ZYX与固定欧拉角XYZ对应
    tf::Quaternion q_odom_cam(Q.x(), Q.y(), Q.z(), Q.w());
    tf::Quaternion q_cam_to_lidar=q_lidar_to_cam.inverse(); // mark: camera - lidar（0，1，0，0）//imu与lidar系重合，则修改为（0001）
