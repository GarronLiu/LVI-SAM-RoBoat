 ...
 * 
 */
double L_I_TX,L_I_TY,L_I_TZ;
double L_I_RX,L_I_RY,L_I_RZ;

tf::Quaternion extQRPY;
...
void readParameters(ros::NodeHandle &n)
{
    ...
    * 
    */
    L_I_TX =fsSettings["lidar_to_cam_tx"];
    L_I_TY =fsSettings["lidar_to_cam_ty"];
    L_I_TZ =fsSettings["lidar_to_cam_tz"];
    L_I_RX =fsSettings["lidar_to_cam_rx"];
    L_I_RY =fsSettings["lidar_to_cam_ry"];
    L_I_RZ =fsSettings["lidar_to_cam_rz"];
    cv::Mat extRPYCV;
    fsSettings["extrinsicRPY"]>>extRPYCV;
    Eigen::Matrix3d extRPY;
    cv::cv2eigen(extRPYCV,extRPY);
    Eigen::Vector3d ypr= extRPY.eulerAngles(2,1,0);
    extQRPY = tf::createQuaternionFromRPY(ypr[2],ypr[1],ypr[0]);
